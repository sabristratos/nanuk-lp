<div>
    @if ($showModal)
        <div x-data="{ showModal: @entangle('showModal') }" x-show="showModal" @keydown.escape.window="showModal = false" class="fixed inset-0 z-50" aria-labelledby="modal-title" role="dialog" aria-modal="true">
            
            {{-- Background overlay --}}
            <div x-show="showModal" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100" x-transition:leave="ease-in duration-200" x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0" class="fixed inset-0 transition-opacity bg-black/75" aria-hidden="true"></div>

            <div class="fixed inset-0 overflow-y-auto flex items-end justify-center px-4 pt-4 pb-20 text-center sm:block sm:p-0">
                {{-- Modal panel --}}
                ...
                <div x-show="showModal"
                     ...
                     class="inline-block w-full max-w-2xl p-6 my-8 overflow-hidden text-left align-middle transition-all transform bg-zinc-950 shadow-xl rounded-2xl"
                     data-element-key="form-modal.panel"
                >
                    ...
                    <div class="flex justify-between items-center pb-3">
                        ...
                        <button wire:click="closeModal" type="button" class="text-zinc-400 hover:text-zinc-300">
                            <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg>
                        </button>
                    </div>

                    @if ($submissionSuccess)
                        <div class="p-4 mb-4 text-sm text-white bg-green-600 rounded-lg" role="alert">
                            <span data-content-key="form-modal.success.title" class="font-medium">Merci !</span> <span data-content-key="form-modal.success.message">Votre demande a été soumise avec succès. Nous vous contacterons bientôt.</span>
                        </div>
                         <div class="mt-6 text-right">
                            <button data-element-key="form-modal.success.close-button" type="button" wire:click="closeModal" class="py-2.5 px-6 rounded-full border-2 border-zinc-600 text-zinc-300 hover:bg-zinc-700 hover:border-zinc-500 focus:outline-none focus:ring-2 focus:ring-zinc-500 focus:ring-offset-2 focus:ring-offset-zinc-800 transition-colors duration-150 ease-in-out font-medium text-sm"><span data-content-key="form-modal.success.close-button-text">Fermer</span></button>
                        </div>
                    @elseif ($submissionFailed)
                        <div class="p-4 mb-4 text-sm text-white bg-red-600 rounded-lg" role="alert">
                            <span data-content-key="form-modal.failure.title" class="font-medium">Erreur :</span> <span data-content-key="form-modal.failure.message">{{ $failureMessage }}</span>
                        </div>
                        <div class="mt-6 text-right">
                            <button data-element-key="form-modal.failure.close-button" type="button" wire:click="closeModal" class="py-2.5 px-6 rounded-full border-2 border-zinc-600 text-zinc-300 hover:bg-zinc-700 hover:border-zinc-500 focus:outline-none focus:ring-2 focus:ring-zinc-500 focus:ring-offset-2 focus:ring-offset-zinc-800 transition-colors duration-150 ease-in-out font-medium text-sm"><span data-content-key="form-modal.failure.close-button-text">Fermer</span></button>
                        </div>
                    @else
                        <form wire:submit.prevent="submit">
                            <div class="space-y-6 mt-4">
                                ...
                                <div class="pt-2">
                                    <div class="flex items-start space-x-3">
                                        <div class="relative flex items-center h-5"> <!-- Ensure h-5 matches checkbox for alignment -->
                            ...
                            <div class="mt-8 pt-5 border-t border-zinc-700 flex justify-end space-x-3">
                                <button data-element-key="form-modal.cancel-button" type="button" wire:click="closeModal" class="py-2.5 px-6 rounded-full border-2 border-zinc-600 text-zinc-300 hover:bg-zinc-700 hover:border-zinc-500 focus:outline-none focus:ring-2 focus:ring-zinc-500 focus:ring-offset-2 focus:ring-offset-zinc-800 transition-colors duration-150 ease-in-out font-medium text-sm"><span data-content-key="form-modal.cancel-button-text">Annuler</span></button>
                                <x-cta-button type="submit" wire:target="submit" data-content-key="form-modal.submit-button-text">
